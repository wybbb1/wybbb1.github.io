...
<p>连接有多种类型，详见下文<a href="#多表查询">“多表查询”</a>部分。</p>
...
### 自然连接(natural join)
自然连接根据两个表中**同名且同类型的列**进行连接，它会自动寻找两个表中名称完全相同的所有列，并基于这些列进行等值匹配。(*不建议使用，可读性较差*)
```bash
# 需要确保两个表中有且只有一个相同名称的列
select {字段名} from {表名1} natural join {表名2};

# 如果只是追求简洁，可以使用USING关键字
# 也是需要确保两个表中有且只有一个相同名称的列，否则可能会存在两个相同名称的列，导致歧义
select {字段名} from {表名1} join {表名2} using ({列名});
```

### 内连接(inner join)
内连接，返回两个表中**满足连接条件**的记录。
如果没有满足连接条件的记录，则不会出现在结果集中。
...
左（右）外连接返回左（右）表的所有记录，以及右（左）表中**满足连接条件**的记录。如果右（左）表中没有匹配的记录，则结果集中对应的字段值为NULL。
这些存在NULL值的元组成为“悬挂元组”。
